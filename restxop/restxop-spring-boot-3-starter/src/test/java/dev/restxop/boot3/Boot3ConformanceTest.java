...
/**
 * Runs the shared wire-level conformance suite against the Boot 3
 * generation's stack (Jackson 2 codec + core engine) — the same suite the
 * Boot 4 starter runs, proving identical wire behavior (SC-005).
 */
class Boot3ConformanceTest extends RestxopConformanceSuite {
    ...
    private RestxopRuntime runtime(WriterSettings settings) {
        ...
        RestxopRuntime runtime = new RestxopRuntime(RestxopConfig.defaults(),
                new Jackson2RootPartCodec(new ObjectMapper()), new FileSpoolStorage(), List.of(),
    ...
    @Override
    protected EncodedMessage encode(Object payload, WriterSettings settings) {
        ...
        return new EncodedMessage(writer.contentType(), out.toByteArray());
    }

    @Override
    protected <T> T decode(String contentType, InputStream body, Type type) {
        return runtime(WriterSettings.fixture())
